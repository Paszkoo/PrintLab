...
const EditFilamentModal = ({ filament, onClose, onSubmit, filterOptions }) => {
  ...
  useEffect(() => {
    if (filament) {
      const data = {
        ...
        priceForKg: filament.priceForKg || 0,
        diameter: filament.diameter ?? '',
        diameterTolerance: filament.diameterTolerance ?? '',
        finish: filament.finish ?? '',
        density: filament.density ?? '',
        heatResistance: filament.heatResistance ?? '',
        shoreHardness: filament.shoreHardness ?? '',
        impactStrength: filament.impactStrength ?? '',
        youngsModulus: filament.youngsModulus ?? '',
        retraction: filament.retraction ?? '',
        nozzleDiameterMin: filament.nozzleDiameterMin ?? '',
        printSpeedMin: filament.printSpeedMin ?? '',
        printSpeedMax: filament.printSpeedMax ?? '',
      };
      setFormData(data);
      const hasAnySpec = [
        data.diameter, data.diameterTolerance, data.finish, data.density,
        data.heatResistance, data.shoreHardness, data.impactStrength,
        data.youngsModulus, data.retraction, data.nozzleDiameterMin,
        data.printSpeedMin, data.printSpeedMax
      ].some(v => v !== '' && v !== null && v !== undefined);
      if (hasAnySpec) setShowSpecs(true);
    }
  }, [filament]);
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.colors.length === 0) {
      alert(t('editfilament.alert.noColor'));
      return;
    }
    ...
    <div className="modal-overlay">
      <div className="modal-container">
        <div className="modal-header">
          <h2>{t('editfilament.title')} {filament.type}</h2>
