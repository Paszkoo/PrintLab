...
const EditPrintModal = ({ model, onClose, onSubmit, filterOptions, filaments = [] }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    margin: 0,
    packageSize: 'medium',
    filamentVariants: [{ filamentProd: '', filamentType: '', colorVariants: [{ color: '', quantity: 1 }] }],
    infill: { infillRate: 20, infillType: 'siatka', vaseWallsCount: 1, vaseBaseLayersCount: 3 },
    supports: { enable: false, angle: 0, type: 'normal' },
    printMode: { vaseMode: false, fuzzyMode: false },
    size: { sizeCorrectionCentimeters: 0 }
  });
  const { t } = useLanguage();
  ...
  useEffect(() => {
    if (model) {
      setFormData({
        ...
        printTimeHours: model.printTimeHours || 0,
        printTimeMinutes: model.printTimeMinutes || 0,
        weightGrams: model.weightGrams || 0,
        margin: model.margin || 0,
        packageSize: model.packageSize || 'medium',
        filamentVariants: Array.isArray(model.filamentVariants) && model.filamentVariants.length > 0
          ? model.filamentVariants.map(fv => {
              const fil = fv.filament || {};
              return {
                filamentProd: fil.prod || '',
                filamentType: fil.type || '',
                colorVariants: Array.isArray(fv.colorVariants)
                  ? fv.colorVariants.map(cv => ({ color: cv.color || '', quantity: Number(cv.quantity) || 0 }))
                  : [{ color: '', quantity: 1 }]
              };
            })
          : [{ filamentProd: '', filamentType: '', colorVariants: [{ color: '', quantity: 1 }] }],
        infill: {
          infillRate: model.infill?.infillRate || 20,
          infillType: model.infill?.infillType || 'siatka',
          vaseWallsCount: model.infill?.vaseWallsCount || 1,
          vaseBaseLayersCount: model.infill?.vaseBaseLayersCount || 3
        },
        supports: {
          enable: model.supports?.enable === true,
          angle: typeof model.supports?.angle === 'number' ? model.supports.angle : 0,
          type: model.supports?.type === 'tree' ? 'tree' : 'normal'
        },
        printMode: {
          vaseMode: model.printMode.vaseMode === true,
          fuzzyMode: model.printMode.fuzzyMode === true
        },
        size: { sizeCorrectionCentimeters: model.size?.sizeCorrectionCentimeters || 0 }
      });
    }
  }, [model]);
    ...
    <div className="modal-overlay">
      <div className="modal-container">
        <div className="modal-header">
          <h2>{t('editprint.title')} {model.name}</h2>
